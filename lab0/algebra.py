    ...
    * expr1 is a Product, and expr2 is a Product
    ...
    if isinstance(expr1, Sum) and isinstance(expr2, Sum):
        ...
        for term1 in expr1:
            ...
            for term2 in expr2:
                #su2 = Sum([term2])
                #newExpr = multiply(su1, su2)
                #for termm in newExpr:
                   # lst.append(termm)
                prod = Product([term1, term2])
                prod = simplify_if_possible(prod)
                lst.append(prod)
        return Sum(lst)
        ...
        for term1 in expr1:
            #su = Sum([term])
            #newExpr = multiply(su, expr2)
            prod = term1
            for term2 in expr2:
                prod = Product([prod, term2])
                prod = simplify_if_possible(prod)
            lst.append(prod)
        return Sum(lst)           
        ...
        for term2 in expr2:
            #su = Sum([term])
            #newExpr = multiply(su, expr1)
            prod = term2
            for term1 in expr1:
                prod = Product([prod, term1])
                prod = simplify_if_possible(prod)
            lst.append(prod)
        return Sum(lst) 
    else:
